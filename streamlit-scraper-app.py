...
# --- データ定義 (変更なし) ---
PREFECTURES = {'東京': 'tokyo', '大阪': 'osaka', '香川': 'kagawa', '北海道': 'hokkaido', '青森': 'aomori',
               '岩手': 'iwate', '宮城': 'miyagi', '秋田': 'akita', '山形': 'yamagata', '福島': 'fukushima',
               '茨城': 'ibaraki', '栃木': 'tochigi', '群馬': 'gunma', '埼玉': 'saitama', '千葉': 'chiba',
               '神奈川': 'kanagawa', '新潟': 'niigata', '富山': 'toyama', '石川': 'ishikawa', '福井': 'fui',
               '山梨': 'yamanashi', '長野': 'nagano', '岐阜': 'gifu', '静岡': 'shizuoka', '愛知': 'aichi',
               '三重': 'mie', '滋賀': 'shiga', '京都': 'kyoto', '兵庫': 'hyogo', '奈良': 'nara', '和歌山': 'wakayama',
               '鳥取': 'tottori', '島根': 'shimane', '岡山': 'okayama', '広島': 'hiroshima', '山口': 'yamaguchi',
               '徳島': 'tokushima', '愛媛': 'ehime', '高知': 'kochi', '福岡': 'fukuoka', '佐賀': 'saga',
...
BUST_MAP = {'Aカップ': 'typ301', 'Bカップ': 'typ302', 'Cカップ': 'typ303', 'Dカップ': 'typ304', 'Eカップ': 'typ305',
            'Fカップ': 'typ306', 'Gカップ': 'typ307', 'Hカップ': 'typ308', 'Iカップ以上': 'typ309'}
...
def create_gallery_html(image_urls):
    """ 画像URLリストからギャラリー用HTMLを生成 """
    if not isinstance(image_urls, list) or not image_urls:
        return ''
    image_paths_json = json.dumps(image_urls)
    return (
        f'<div class="gallery-container" data-images=\'{image_paths_json}\' data-current-index="0" onclick="nextImage(this)" style="cursor: pointer;">'
        f'<img src="{image_urls[0]}" width="120" onerror="this.style.display=\'none\'">'
        f'</div>'
    )


def create_star_rating_html(sort_number):
    """ 元のtkinter版と同じHTMLを生成 """
    try:
        num = int(sort_number)
...
def parse_style(style_text):
    ...
    if style_text and (match := re.search(r'T(\d+).*?･(\d+)\((.*?)\)･(\d+)･(\d+)', style_text)):
        try:
            data.update({"身長(cm)": int(match.group(1)), "バスト(cm)": int(match.group(2)), "カップ": match.group(3),
                         "ウェスト(cm)": int(match.group(4)), "ヒップ(cm)": int(match.group(5))})
...
def get_girl_details(profile_url, session, headers):
    details = {"本日の出勤予定": None, "次回出勤": None, "ギャラリーURL": [], "WEB人気の星": 999, "週合計出勤日数": 0,
               "週合計勤務時間": 0.0, "口コミ数": 0}
    ...
    try:
        ...
        details["ギャラリーURL"] = list(dict.fromkeys(image_urls))[:6]  # 最大6枚まで
        ...
        if star_img_tag := soup.find('img', class_='yoyaku_girlmark'):
            if match := re.search(r'yoyaku_(\d+)\.png', star_img_tag.get('src', '')): details["WEB人気の星"] = int(
                match.group(1))
        ...
        try:
            ...
            for page in range(1, pages_to_scrape + 1):
                ...
                for item in soup.find_all('li', class_='girls-list'):
                    ...
                    data = {"名前": n.text.strip() if (n := item.find('p', 'girlname').find('a')) else "NoName",
                            "年齢": parse_age(a.text.strip() if (a := item.find('p', 'girlname').find('span')) else ''),
                            "出勤状況": status, "次回出勤": parse_sortable_time(status),
                            **parse_style(s.text.strip() if (s := item.find('p', 'girlstyle')) else ''),
                            "プロフィールリンク": profile_link,
                            "店舗名": (s.text.strip() if (s := item.find('p', 'shopname').find('a')) else None)}
                    initial_girl_list.append(data)
                time.sleep(0.5)

            status_text.text(f"全{len(initial_girl_list)}人の詳細情報を並列取得中...")
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                future_to_data = {executor.submit(get_girl_details, data['プロフィールリンク'], session, headers): data
                                  for data in initial_girl_list}
                progress_count = 0
            ...
            if not all_girls_data:
                status_text.text("データが取得できませんでした。")
                return pd.DataFrame()
...
def generate_html_report(df, title_text):
    ...
    # 表示用にデータを変換
    df_display['名前'] = df_display.apply(
        lambda row: f'<a href="{row["プロフィールリンク"]}" target="_blank">{row["名前"]}</a>' if pd.notna(
            row['プロフィールリンク']) else row['名前'], axis=1)
    ...
    desired_order = ["名前", "ギャラリー", "年齢", "身長", "カップ", "WEB人気", "口コミ数", "出勤状況",
                     "本日の出勤予定", "次回出勤", "バスト", "ウェスト", "ヒップ", "店舗名"]
    existing_columns = [col for col in desired_order if col in df_display.columns]
    df_display = df_display[existing_columns]

    # HTMLテーブルの生成
    html_table = df_display.to_html(escape=False, index=False, table_id='resultsTable',
...
with st.sidebar:
    ...
    with st.expander("特徴で絞り込み"):
        selected_features = []
        st.subheader("年齢");
        [selected_features.append(name) for name in AGE_MAP.keys() if st.checkbox(name, value=True)]
        st.subheader("身長");
        [selected_features.append(name) for name in HEIGHT_MAP.keys() if st.checkbox(name, value=True)]
        st.subheader("バスト");
        [selected_features.append(name) for name in BUST_MAP.keys() if st.checkbox(name)]
        st.subheader("その他特徴");
        [selected_features.append(name) for name in FEATURE_MAP.keys() if st.checkbox(name)]
    st.header("オプション")
...
if start_button:
    ...
    params = (PREFECTURES[prefecture_name], filter_path, debug_mode, hide_inactive,
              None if page_limit == '全て' else int(page_limit))
    st.subheader("処理状況")
    ...
    if not st.session_state.result_df.empty:
        ...
        with tab2:
            st.dataframe(
                ...
                column_config={"ギャラリーURL": st.column_config.ImageColumn("Photo"),
                               "プロフィールリンク": st.column_config.LinkColumn("Profile Link")},
                use_container_width=True
            )

        csv = st.session_state.result_df.to_csv(index=False).encode('utf-8-sig')
        st.download_button(label="結果をCSVでダウンロード", data=csv,
                           file_name=f"heaven_data_{prefecture_name}_{int(time.time())}.csv", mime="text/csv")
    else:
